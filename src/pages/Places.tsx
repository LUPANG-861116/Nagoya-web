        ...
        <>
          <section>
            ...
            <div className="cards-grid">
              {fSpots.map((p, i) => {
                const matchImg = p.body?.match(/!\[.*?\]\((https?:\/\/[^\s)]+)\)/);
                const coverUrl = matchImg ? matchImg[1] : null;
                return (
                  <Reveal key={p.id} index={i}>
                  <div className="card" style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {coverUrl && (
                      <div style={{ width: '100%', height: 130, borderRadius: 8, overflow: 'hidden', position: 'relative' }}>
                        <img src={coverUrl} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        <span style={{ position: 'absolute', right: 8, bottom: 6, fontSize: 10.5, fontWeight: 600, background: 'rgba(25,20,15,0.72)', color: '#F7F2E6', padding: '2px 8px', borderRadius: 4, backdropFilter: 'blur(4px)' }}>
                          浪漫散策
                        </span>
                      </div>
                    )}
          ...
          <section>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
              <span className="serif" style={{ fontSize: 20, fontWeight: 800, borderLeft: '4px solid var(--navy)', paddingLeft: 12 }}>購物・商場</span>
              <span style={{ fontSize: 12, color: 'var(--brown)' }}>高島屋・中日大樓・松坂屋・戶外裝備採買</span>
            </div>
            {fShops.length === 0 && (
